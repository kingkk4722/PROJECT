...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Save mobile number and email in user profile or other associated model
            user_profile = UserProfile.objects.create(user=user,mobile_number=form.cleaned_data['mobile_number'],
...
@login_required(login_url='login')
def verify_otp(request, ownership_id):
    ownership = get_object_or_404(Ownership, id=ownership_id)

    if request.method == 'POST':
        entered_otp = request.POST.get('otp')

        if verify_otp(entered_otp, ownership.transfer_otp):
            # OTP verification successful, proceed with ownership transfer
            new_owner_username = ownership.new_owner_username
            ownership.owner.username = new_owner_username
            ownership.owner.save()

            messages.success(request, "Ownership transferred successfully.")
            return redirect('account_home')  # or any other appropriate redirect
        else:
            messages.error(request, "Invalid OTP. Ownership transfer failed.")

    return render(request, 'ownership_transfer/verify_otp.html', {'ownership': ownership})
def home(request):
    if request.method == 'POST':
